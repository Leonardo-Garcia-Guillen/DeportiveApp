...
import controlador.CrossfitBooking;
...
public class ReservaCrossfit implements ActionListener {
	CrossfitBooking crossfit = new CrossfitBooking();

	public static ChangeWindow change = new ChangeWindow("");
	public static ChangePanelHours panelHours = new ChangePanelHours();

	private final int sportNumber = crossfit.getSportNumber();
	private final int totalDays = 7;
	private final int totalHours = crossfit.getTotalHours();
	private final int totalUsers = crossfit.getTotalUsers();
	private final int totalSchedules = crossfit.getTotalSchedules();
	 ...
	 */
	public void newCrossfitInterface() {

		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ReservaCrossfit window = new ReservaCrossfit();
	...
	private void initialize() {
		...
		ImageIcon imageIcon = new ImageIcon(new ImageIcon(InterfazPrincipal.class.getResource("/images/crossfit.jpeg"))
		...
		JLabel lblTitulo = new JLabel("CROSSFIT");
		...
		// Ayudas de colores
		JLabel lblHelpGreen = new JLabel("Borde verde: box libre");
		...
		JLabel lblHelpRed = new JLabel("Borde rojo: box totalmente ocupado");
		...
		JLabel lblHelpOrange = new JLabel("Borde naranja: box parcialmente ocupado");
	...
	private void getHourBtns() {
		...
		for (int day = 0; day < 7; day++) {
			weekYear = crossfit.getCrossfitWeek().getDayBtn(day).getWeekYear();
			for (int schedule = 0; schedule < totalSchedules; schedule++) {
				for (int hour = 0; hour < totalHours; hour++) {
					btnHoursArray[day][schedule][hour] = new JButton(crossfit.getCrossfitWeek().getDayBtn(day)
							.getSchedules().get(schedule).getSchedule().get(hour).getHourStr());
					// Nombre: 0 (crossfit) + nº de la semana respecto del año + dia + pista + hora
	...
	private void getDayBtns(JPanel panel) {
		for (int i = 0; i < totalDays; i++) {
			...
			btnDaysArray[i].setText(crossfit.getCrossfitWeek().getDayBtn(i).getDayName().toUpperCase());
