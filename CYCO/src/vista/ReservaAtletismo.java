...
import controlador.AtletismoBooking;
...
public class ReservaAtletismo implements ActionListener {
	AtletismoBooking atletismo = new AtletismoBooking();

	public static ChangeWindow change = new ChangeWindow("");
	public static ChangePanelHours panelHours = new ChangePanelHours();

	private final int sportNumber = atletismo.getSportNumber();
	private final int totalDays = 7;
	private final int totalHours = atletismo.getTotalHours();
	private final int totalUsers = atletismo.getTotalUsers();
	private final int totalSchedules = atletismo.getTotalSchedules();
	 ...
	 */
	public void newAtletismoInterface() {

		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ReservaAtletismo window = new ReservaAtletismo();
	...
	private void initialize() {
		...
		ImageIcon imageIcon = new ImageIcon(new ImageIcon(InterfazPrincipal.class.getResource("/images/atletismo.jpg"))
		...
		JLabel lblTitulo = new JLabel("ATLETISMO");
	...
	private void getHourBtns() {
		...
		for (int day = 0; day < 7; day++) {
			weekYear = atletismo.getAtletismoWeek().getDayBtn(day).getWeekYear();
			for (int schedule = 0; schedule < totalSchedules; schedule++) {
				for (int hour = 0; hour < totalHours; hour++) {
					btnHoursArray[day][schedule][hour] = new JButton(atletismo.getAtletismoWeek().getDayBtn(day)
							.getSchedules().get(schedule).getSchedule().get(hour).getHourStr());
					// Nombre: 0 (atletismo) + nº de la semana respecto del año + dia + pista + hora
					...
					btnHoursArray[day][schedule][hour].setBounds((200 + x * 820), 10 + y * 64, 139, 32);
					y++;
	...
	private void getDayBtns(JPanel panel) {
		for (int i = 0; i < totalDays; i++) {
			...
			btnDaysArray[i].setText(atletismo.getAtletismoWeek().getDayBtn(i).getDayName().toUpperCase());
